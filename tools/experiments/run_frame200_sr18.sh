...
# sr-18: sr-7 plus strong-PCA orientation for dynamic raw-support seeds.
...
MARKER="${CONTROL_ROOT}/sr18_inference_started"

mkdir -p "${CONTROL_ROOT}"
touch "${MARKER}"
trap 'rm -f "${MARKER}"' EXIT

echo "stage=sr18_inference status=starting timestamp=$(date --iso-8601=seconds)"
cd "${WORKSPACE}"
"${PYTHON}" reconstructed_inference.py \
    ...
    --dynamic_expand_adaptive_axis true --dynamic_expand_axis_radius 3 \
    --dynamic_expand_min_axis_ratio 10 --dynamic_expand_keep_longitudinal false \
...
echo "stage=sr18_inference status=verified expected=${expected_count} outputs=${output_count} stale=${stale_count}"
if [[ "${output_count}" -ne "${expected_count}" || "${stale_count}" -ne 0 ]]; then
    echo "sr-18 inference inventory verification failed" >&2
    exit 1
fi

echo "stage=sr18_info_rebuild status=starting timestamp=$(date --iso-8601=seconds)"
cd "${REPO_ROOT}"
...
echo "stage=sr18_training status=starting timestamp=$(date --iso-8601=seconds)"
cd "${REPO_ROOT}/tools"
/usr/bin/env \
    ...
    --extra_tag frame200_sr18_350m --fix_random_seed --workers 2 --use_amp \
...
echo "stage=sr18_pipeline status=completed timestamp=$(date --iso-8601=seconds)"
