...
# sr-33: sr-7 geometry with a provenance-specific RCS scale on PCA-lateral
# dense support only. Dynamic and ordinary dense synthetic points stay exact.
...
MARKER="${CONTROL_ROOT}/sr33_inference_started"

mkdir -p "${CONTROL_ROOT}"
touch "${MARKER}"
trap 'rm -f "${MARKER}"' EXIT

echo "stage=sr33_inference status=starting timestamp=$(date --iso-8601=seconds)"
cd "${WORKSPACE}"
"${PYTHON}" reconstructed_inference.py \
    ...
    --dense_expand_adaptive_rcs_scale 1.25
...
echo "stage=sr33_inference status=verified expected=${expected_count} outputs=${output_count} stale=${stale_count}"
if [[ "${output_count}" -ne "${expected_count}" || "${stale_count}" -ne 0 ]]; then
    echo "sr-33 inference inventory verification failed" >&2
    exit 1
fi

echo "stage=sr33_info_rebuild status=starting timestamp=$(date --iso-8601=seconds)"
cd "${REPO_ROOT}"
...
echo "stage=sr33_training status=starting timestamp=$(date --iso-8601=seconds)"
cd "${REPO_ROOT}/tools"
/usr/bin/env PATH="/home/kaixuan-ding/miniconda3/bin:/home/kaixuan-ding/miniconda3/envs/spconv/bin:/usr/local/bin:/usr/bin:/bin" \
    ...
    --extra_tag frame200_sr33_350m --fix_random_seed --workers 2 --use_amp \
...
echo "stage=sr33_pipeline status=completed timestamp=$(date --iso-8601=seconds)"
