...
# sr-47: regenerate the sr-45 best fill, then train a strict raw/SR pair in
# FP32 to audit whether AMP overflow sensitivity masks the fill benefit.
...
MARKER="${CONTROL_ROOT}/sr47_inference_started"

mkdir -p "${CONTROL_ROOT}"
touch "${MARKER}"
trap 'rm -f "${MARKER}"' EXIT

echo "stage=sr47_inference status=starting timestamp=$(date --iso-8601=seconds)"
cd "${WORKSPACE}"
"${PYTHON}" reconstructed_inference.py \
    ...
    --dynamic_expand_rcs_scale 0.25 --dynamic_expand_absv_scale 1.0 \
...
echo "stage=sr47_inference status=verified expected=${expected_count} outputs=${output_count} stale=${stale_count}"
if [[ "${output_count}" -ne "${expected_count}" || "${stale_count}" -ne 0 ]]; then
    echo "sr-47 inference inventory verification failed" >&2
    exit 1
fi

echo "stage=sr47_info_rebuild status=starting timestamp=$(date --iso-8601=seconds)"
cd "${REPO_ROOT}"
...
common_train_args=(
    ...
    --fix_random_seed --workers 2
    --logger_iter_interval 20 --structured_log_iter_interval 10
)

echo "stage=sr47_raw_fp32_training status=starting timestamp=$(date --iso-8601=seconds)"
cd "${REPO_ROOT}/tools"
/usr/bin/env PATH="/home/kaixuan-ding/miniconda3/bin:/home/kaixuan-ding/miniconda3/envs/spconv/bin:/usr/local/bin:/usr/bin:/bin" \
    ...
    --extra_tag frame200_raw_sr47_fp32_350m \
    --set DATA_CONFIG.INFO_PATH.train "${INFO_ROOT}/raw_train.pkl" \
    DATA_CONFIG.INFO_PATH.test "${INFO_ROOT}/raw_val.pkl"

echo "stage=sr47_enhanced_fp32_training status=starting timestamp=$(date --iso-8601=seconds)"
/usr/bin/env PATH="/home/kaixuan-ding/miniconda3/bin:/home/kaixuan-ding/miniconda3/envs/spconv/bin:/usr/local/bin:/usr/bin:/bin" \
    "${PYTHON}" train.py "${common_train_args[@]}" \
    --extra_tag frame200_sr47_fp32_350m \
    --set DATA_CONFIG.INFO_PATH.train "${INFO_ROOT}/sr_train.pkl" \
    DATA_CONFIG.INFO_PATH.test "${INFO_ROOT}/sr_val.pkl"

echo "stage=sr47_pipeline status=completed timestamp=$(date --iso-8601=seconds)"
