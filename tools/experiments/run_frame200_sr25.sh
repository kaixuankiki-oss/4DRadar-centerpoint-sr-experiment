...
# sr-25: sr-6 with positive-only dynamic longitudinal support.
...
MARKER="${CONTROL_ROOT}/sr25_inference_started"
mkdir -p "${CONTROL_ROOT}"
touch "${MARKER}"
trap 'rm -f "${MARKER}"' EXIT

echo "stage=sr25_inference status=starting timestamp=$(date --iso-8601=seconds)"
cd "${WORKSPACE}"
"${PYTHON}" reconstructed_inference.py \
    ...
    --dynamic_expand_direction positive --expand_dense_raw true \
...
echo "stage=sr25_inference status=verified expected=${expected_count} outputs=${output_count} stale=${stale_count}"
if [[ "${output_count}" -ne "${expected_count}" || "${stale_count}" -ne 0 ]]; then
    echo "sr-25 inference inventory verification failed" >&2
    exit 1
fi

echo "stage=sr25_info_rebuild status=starting timestamp=$(date --iso-8601=seconds)"
cd "${REPO_ROOT}"
...
echo "stage=sr25_training status=starting timestamp=$(date --iso-8601=seconds)"
cd "${REPO_ROOT}/tools"
/usr/bin/env PATH="/home/kaixuan-ding/miniconda3/bin:/home/kaixuan-ding/miniconda3/envs/spconv/bin:/usr/local/bin:/usr/bin:/bin" \
    ...
    --extra_tag frame200_sr25_350m --fix_random_seed --workers 2 --use_amp \
...
echo "stage=sr25_pipeline status=completed timestamp=$(date --iso-8601=seconds)"
