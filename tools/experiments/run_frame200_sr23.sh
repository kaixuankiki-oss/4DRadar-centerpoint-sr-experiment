...
# sr-23: sr-6 support with a local-consistency gate on dynamic source cells.
# This overwrites the shared SR PCD root, rebuilds info PKLs, and runs the
# fixed 350m CenterPoint training pipeline under frame200_job_control.sh.
...
MARKER="${CONTROL_ROOT}/sr23_inference_started"

mkdir -p "${CONTROL_ROOT}"
touch "${MARKER}"
trap 'rm -f "${MARKER}"' EXIT

echo "stage=sr23_inference status=starting timestamp=$(date --iso-8601=seconds)"
cd "${WORKSPACE}"
"${PYTHON}" reconstructed_inference.py \
    ...
    --dynamic_expand_min_points 2 \
    --dynamic_expand_require_neighbor true \
...
echo "stage=sr23_inference status=verified expected=${expected_count} outputs=${output_count} stale=${stale_count}"
if [[ "${output_count}" -ne "${expected_count}" || "${stale_count}" -ne 0 ]]; then
    echo "sr-23 inference inventory verification failed" >&2
    exit 1
fi

echo "stage=sr23_info_rebuild status=starting timestamp=$(date --iso-8601=seconds)"
cd "${REPO_ROOT}"
...
echo "stage=sr23_training status=starting timestamp=$(date --iso-8601=seconds)"
cd "${REPO_ROOT}/tools"
/usr/bin/env PATH="/home/kaixuan-ding/miniconda3/bin:/home/kaixuan-ding/miniconda3/envs/spconv/bin:/usr/local/bin:/usr/bin:/bin" \
    ...
    --extra_tag frame200_sr23_350m --fix_random_seed --workers 2 --use_amp \
...
echo "stage=sr23_pipeline status=completed timestamp=$(date --iso-8601=seconds)"
