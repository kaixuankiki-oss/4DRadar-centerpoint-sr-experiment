...
# sr-36: half-scale local support grid with sr-7 PCA dense orientation.
...
MARKER="${CONTROL_ROOT}/sr36_inference_started"

mkdir -p "${CONTROL_ROOT}"
touch "${MARKER}"
trap 'rm -f "${MARKER}"' EXIT

echo "stage=sr36_inference status=starting timestamp=$(date --iso-8601=seconds)"
cd "${WORKSPACE}"
"${PYTHON}" reconstructed_inference.py \
    ...
    --raw_expand_max_range 50 --raw_expand_voxel_size 0.125 0.10 \
    --expand_dense_raw true \
    --dense_expand_min_points 4 --dense_expand_min_rcs 5 \
...
echo "stage=sr36_inference status=verified expected=${expected_count} outputs=${output_count} stale=${stale_count}"
if [[ "${output_count}" -ne "${expected_count}" || "${stale_count}" -ne 0 ]]; then
    echo "sr-36 inference inventory verification failed" >&2
    exit 1
fi

echo "stage=sr36_info_rebuild status=starting timestamp=$(date --iso-8601=seconds)"
cd "${REPO_ROOT}"
...
echo "stage=sr36_training status=starting timestamp=$(date --iso-8601=seconds)"
cd "${REPO_ROOT}/tools"
/usr/bin/env PATH="/home/kaixuan-ding/miniconda3/bin:/home/kaixuan-ding/miniconda3/envs/spconv/bin:/usr/local/bin:/usr/bin:/bin" \
    ...
    --extra_tag frame200_sr36_350m --fix_random_seed --workers 2 --use_amp \
...
echo "stage=sr36_pipeline status=completed timestamp=$(date --iso-8601=seconds)"
