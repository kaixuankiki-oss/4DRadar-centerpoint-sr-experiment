...
# sr-26: sr-6 deterministic support plus sparse learned occupancy points.
...
MARKER="${CONTROL_ROOT}/sr26_inference_started"
mkdir -p "${CONTROL_ROOT}"
touch "${MARKER}"
trap 'rm -f "${MARKER}"' EXIT

echo "stage=sr26_inference status=starting timestamp=$(date --iso-8601=seconds)"
cd "${WORKSPACE}"
"${PYTHON}" reconstructed_inference.py \
    ...
    --threshold 0.99 --add_offset true --use_aai false \
    --use_original_overlay false --add_is_sr true --use_neighborhood_filling true \
    --preserve_original_points true --sr_min_rcs 2 --sr_max_range 50 \
...
echo "stage=sr26_inference status=verified expected=${expected_count} outputs=${output_count} stale=${stale_count}"
if [[ "${output_count}" -ne "${expected_count}" || "${stale_count}" -ne 0 ]]; then
    echo "sr-26 inference inventory verification failed" >&2
    exit 1
fi

echo "stage=sr26_info_rebuild status=starting timestamp=$(date --iso-8601=seconds)"
cd "${REPO_ROOT}"
...
echo "stage=sr26_training status=starting timestamp=$(date --iso-8601=seconds)"
cd "${REPO_ROOT}/tools"
/usr/bin/env PATH="/home/kaixuan-ding/miniconda3/bin:/home/kaixuan-ding/miniconda3/envs/spconv/bin:/usr/local/bin:/usr/bin:/bin" \
    ...
    --extra_tag frame200_sr26_350m --fix_random_seed --workers 2 --use_amp \
...
echo "stage=sr26_pipeline status=completed timestamp=$(date --iso-8601=seconds)"
