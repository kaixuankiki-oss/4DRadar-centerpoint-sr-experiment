...
# sr-42: sr-7 geometry/features, dense synthetic z uses source-voxel median.
...
MARKER="${CONTROL_ROOT}/sr42_inference_started"

mkdir -p "${CONTROL_ROOT}"
touch "${MARKER}"
trap 'rm -f "${MARKER}"' EXIT

echo "stage=sr42_inference status=starting timestamp=$(date --iso-8601=seconds)"
cd "${WORKSPACE}"
"${PYTHON}" reconstructed_inference.py \
    ...
    --dense_expand_z_mode voxel_median
...
echo "stage=sr42_inference status=verified expected=${expected_count} outputs=${output_count} stale=${stale_count}"
if [[ "${output_count}" -ne "${expected_count}" || "${stale_count}" -ne 0 ]]; then
    echo "sr-42 inference inventory verification failed" >&2
    exit 1
fi

echo "stage=sr42_info_rebuild status=starting timestamp=$(date --iso-8601=seconds)"
cd "${REPO_ROOT}"
...
echo "stage=sr42_training status=starting timestamp=$(date --iso-8601=seconds)"
cd "${REPO_ROOT}/tools"
/usr/bin/env PATH="/home/kaixuan-ding/miniconda3/bin:/home/kaixuan-ding/miniconda3/envs/spconv/bin:/usr/local/bin:/usr/bin:/bin" \
    ...
    --extra_tag frame200_sr42_350m --fix_random_seed --workers 2 --use_amp \
...
echo "stage=sr42_pipeline status=completed timestamp=$(date --iso-8601=seconds)"
