...
# sr-48: regenerate sr-45 fill, then run a strict seed-42 AMP raw/SR pair.
...
MARKER="${CONTROL_ROOT}/sr48_inference_started"

mkdir -p "${CONTROL_ROOT}"
touch "${MARKER}"
trap 'rm -f "${MARKER}"' EXIT

echo "stage=sr48_inference status=starting timestamp=$(date --iso-8601=seconds)"
cd "${WORKSPACE}"
...
echo "stage=sr48_inference status=verified expected=${expected_count} outputs=${output_count} stale=${stale_count}"
if [[ "${output_count}" -ne "${expected_count}" || "${stale_count}" -ne 0 ]]; then
    echo "sr-48 inference inventory verification failed" >&2
    exit 1
fi

echo "stage=sr48_info_rebuild status=starting timestamp=$(date --iso-8601=seconds)"
cd "${REPO_ROOT}"
...
common_train_args=(
    ...
    --fix_random_seed --random_seed 42 --workers 2 --use_amp
    --logger_iter_interval 20 --structured_log_iter_interval 10
)

echo "stage=sr48_raw_seed42_training status=starting timestamp=$(date --iso-8601=seconds)"
cd "${REPO_ROOT}/tools"
/usr/bin/env PATH="/home/kaixuan-ding/miniconda3/bin:/home/kaixuan-ding/miniconda3/envs/spconv/bin:/usr/local/bin:/usr/bin:/bin" \
    ...
    --extra_tag frame200_raw_sr48_seed42_350m \
    --set DATA_CONFIG.INFO_PATH.train "${INFO_ROOT}/raw_train.pkl" \
    DATA_CONFIG.INFO_PATH.test "${INFO_ROOT}/raw_val.pkl"

echo "stage=sr48_enhanced_seed42_training status=starting timestamp=$(date --iso-8601=seconds)"
/usr/bin/env PATH="/home/kaixuan-ding/miniconda3/bin:/home/kaixuan-ding/miniconda3/envs/spconv/bin:/usr/local/bin:/usr/bin:/bin" \
    "${PYTHON}" train.py "${common_train_args[@]}" \
    --extra_tag frame200_sr48_seed42_350m \
    --set DATA_CONFIG.INFO_PATH.train "${INFO_ROOT}/sr_train.pkl" \
    DATA_CONFIG.INFO_PATH.test "${INFO_ROOT}/sr_val.pkl"

echo "stage=sr48_pipeline status=completed timestamp=$(date --iso-8601=seconds)"
