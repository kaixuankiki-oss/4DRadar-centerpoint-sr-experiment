...
# sr-17: sr-7 geometry/gates with source-cell-relative synthetic coordinates.
...
MARKER="${CONTROL_ROOT}/sr17_inference_started"

mkdir -p "${CONTROL_ROOT}"
touch "${MARKER}"
trap 'rm -f "${MARKER}"' EXIT

echo "stage=sr17_inference status=starting timestamp=$(date --iso-8601=seconds)"
cd "${WORKSPACE}"
"${PYTHON}" reconstructed_inference.py \
    ...
    --raw_expand_coordinate_mode copy_offset \
...
echo "stage=sr17_inference status=verified expected=${expected_count} outputs=${output_count} stale=${stale_count}"
if [[ "${output_count}" -ne "${expected_count}" || "${stale_count}" -ne 0 ]]; then
    echo "sr-17 inference inventory verification failed" >&2
    exit 1
fi

echo "stage=sr17_info_rebuild status=starting timestamp=$(date --iso-8601=seconds)"
cd "${REPO_ROOT}"
...
echo "stage=sr17_training status=starting timestamp=$(date --iso-8601=seconds)"
cd "${REPO_ROOT}/tools"
/usr/bin/env \
    ...
    --extra_tag frame200_sr17_350m --fix_random_seed --workers 2 --use_amp \
...
echo "stage=sr17_pipeline status=completed timestamp=$(date --iso-8601=seconds)"
