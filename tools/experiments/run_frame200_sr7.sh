...
# sr-7: retain sr-6 gates, but orient dense-slow expansion with local PCA.
...
MARKER="${CONTROL_ROOT}/sr7_inference_started"

mkdir -p "${CONTROL_ROOT}"
touch "${MARKER}"
trap 'rm -f "${MARKER}"' EXIT

echo "stage=sr7_inference status=starting timestamp=$(date --iso-8601=seconds)"
cd "${WORKSPACE}"
...
echo "stage=sr7_inference status=verified expected=${expected_count} outputs=${output_count} stale=${stale_count}"
if [[ "${output_count}" -ne "${expected_count}" || "${stale_count}" -ne 0 ]]; then
    echo "sr-7 inference inventory verification failed" >&2
    exit 1
fi

echo "stage=sr7_info_rebuild status=starting timestamp=$(date --iso-8601=seconds)"
cd "${REPO_ROOT}"
...
echo "stage=sr7_training status=starting timestamp=$(date --iso-8601=seconds)"
cd "${REPO_ROOT}/tools"
/usr/bin/env \
    ...
    --extra_tag frame200_sr7_350m --fix_random_seed --workers 2 --use_amp \
...
echo "stage=sr7_pipeline status=completed timestamp=$(date --iso-8601=seconds)"
