...
class DataBaseSampler(object):
    def __init__(self, root_path, sampler_cfg, class_names, logger=None, storage=None):
        ...
        for db_info_path in sampler_cfg.DB_INFO_PATH:
            if not self._exists(db_info_path):
            ...
            infos = self._load_pickle(db_info_path)
    ...
    def _resolve_file_path(self, path):
        return self.root_path.resolve() / path
    ...
    def _exists(self, path):
        ...
        return self._resolve_file_path(path).exists()

    def _load_pickle(self, path):
        if self.storage is not None:
            return self.storage.load_pickle(path)
        with open(str(self._resolve_file_path(path)), 'rb') as f:
            return pickle.load(f)

    def _np_load(self, path):
        if self.storage is not None:
            return self.storage.np_load(path)
        return np.load(self._resolve_file_path(path))

    def _fromfile(self, path, dtype):
        if self.storage is not None:
            return self.storage.fromfile(path, dtype=dtype)
        return np.fromfile(str(self._resolve_file_path(path)), dtype=dtype)
    ...
    def load_db_to_shared_memory(self):
        ...
        db_data_path = self.sampler_cfg.DB_DATA_PATH[0]
        sa_key = self.sampler_cfg.DB_DATA_PATH[0]

        if cur_rank % num_gpus == 0 and not os.path.exists(f"/dev/shm/{sa_key}"):
            gt_database_data = self._np_load(db_data_path)
    ...
    def add_sampled_boxes_to_scene(self, data_dict, sampled_gt_boxes, total_valid_sampled_dict, mv_height=None, sampled_gt_boxes2d=None):
        ...
        for idx, info in enumerate(total_valid_sampled_dict):
            if self.use_shared_memory:
                ...
                obj_points = self._fromfile(info['path'], dtype=np.float32).reshape(
                    [-1, self.sampler_cfg.NUM_POINT_FEATURES])
                if obj_points.shape[0] != info['num_points_in_gt']:
                    obj_points = self._fromfile(info['path'], dtype=np.float64).reshape(-1, self.sampler_cfg.NUM_POINT_FEATURES)
