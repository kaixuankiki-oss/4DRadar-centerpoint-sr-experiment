# exp28_rcs_only_temporal_consistency

Optional isolation variant using RCS/power/range/timestamp temporal consistency without radial residual features.
...
CUDA_VISIBLE_DEVICES=4,5,6,7 torchrun --standalone --nproc_per_node=4 train.py \
  --launcher pytorch \
  --cfg_file cfgs/radar_models/exp28_rcs_only_temporal_consistency.yaml \
  --extra_tag exp28_rcs_only_temporal_consistency_4gpu_40ep_20260620 \
  --batch_size 128 \
  --workers 8 \
...
```bash
cd /workspace/worktrees/a100_radial_rcs_consistency_20260620/tools
CUDA_VISIBLE_DEVICES=4 python tools/experiments/smoke_one_batch_radial_wing.py --cfg_file cfgs/radar_models/exp28_rcs_only_temporal_consistency.yaml --batch_size 1 --workers 1
```
