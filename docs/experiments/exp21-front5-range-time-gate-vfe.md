# Experiment 21: RADAR_FRONT 5-sweep range/time gate VFE

## Objective

Test whether a point-level range/time/RCS/Doppler gate can make 5-sweep radar
pillars more robust to sparse far-range returns and sweep-age noise.

## Hypothesis

Far-range radar evidence is sparse and diffused. A learned point gate can
down-weight weak or stale point evidence while keeping geometry, BEV size and
CenterHead targets unchanged.

## Model Change

- Config: `tools/cfgs/radar_models/exp21_front5_range_time_gate_vfe.yaml`
- VFE: `RangeTimeGatePillarVFE`
- Gate inputs: range, timestamp, RCS, Doppler and power.
- The gate scales radar evidence features only; xyz coordinates and timestamp
  are preserved. `Vx,Vy` are not used as object-motion compensation.
...
python experiments/smoke_one_batch_radial_wing.py \
  --cfg_file cfgs/radar_models/exp21_front5_range_time_gate_vfe.yaml \
...
CUDA_VISIBLE_DEVICES=4,5,6,7 torchrun --standalone --nproc_per_node=4 train.py \
  --launcher pytorch \
  --cfg_file cfgs/radar_models/exp21_front5_range_time_gate_vfe.yaml \
  --extra_tag exp21_front5_range_time_gate_vfe_4gpu_40ep_20260619 \
  --batch_size 128 \
  --workers 8 \
