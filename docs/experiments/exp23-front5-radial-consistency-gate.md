# Experiment 23: RADAR_FRONT 5-sweep radial-consistency VFE gate

## Objective

Exploit the fact that radar point velocity is radial while avoiding false
two-dimensional object-motion compensation from `Vx,Vy`.

## Hypothesis

The consistency between raw Doppler and the x/y decomposed radial velocity can
identify unstable or wing-diffused points. A point gate using this residual can
improve long-range sparse evidence aggregation without changing BEV or targets.

## Model Change

- Config: `tools/cfgs/radar_models/exp23_front5_radial_consistency_gate.yaml`
- VFE: `RadialConsistencyPillarVFE`
- Inputs: exp21 gate inputs plus radial-from-`Vx,Vy`, radial residual and
  tangential residual magnitude.
- `Vx,Vy` are interpreted only as radial velocity decompositions. No pseudo
  object-motion compensation is performed.
...
python experiments/smoke_one_batch_radial_wing.py \
  --cfg_file cfgs/radar_models/exp23_front5_radial_consistency_gate.yaml \
...
CUDA_VISIBLE_DEVICES=4,5,6,7 torchrun --standalone --nproc_per_node=4 train.py \
  --launcher pytorch \
  --cfg_file cfgs/radar_models/exp23_front5_radial_consistency_gate.yaml \
  --extra_tag exp23_front5_radial_consistency_gate_4gpu_40ep_20260619 \
  --batch_size 128 \
  --workers 8 \
